...
pub(super) fn key_mods(mods: KeyButMask) -> Modifiers {
    ...
    let key_masks = [
        ...
        //
        // SpectralPrism fix - see this vendored copy's README.md: these
        // three were `KeyButMask::BUTTON1`/`BUTTON2`/`BUTTON4` (the current
        // *mouse button* state - bits 8/9/11) instead of `MOD1`/`MOD2`/
        // `MOD4` (the actual Alt/NumLock/Super *modifier key* state - bits
        // 3/4/6), a real upstream bug: X11's `KeyButMask` combines both key
        // and button state into one bitmask, and this picked the wrong
        // half. In practice this meant Alt (and therefore, in `egui`'s own
        // `TextEdit`, "delete previous word" instead of "delete previous
        // character" on Backspace) got spuriously reported as held any time
        // the left mouse button happened to be down at the same moment - as
        // it very often still is right after clicking into a text field to
        // focus it, before immediately typing.
