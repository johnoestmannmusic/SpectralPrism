...
impl XcbConnection {
    pub fn new() -> Result<Self, Box<dyn Error>> {
        ...
        assert!(!dpy.is_null());
        // Without this, X11's classic (non-"detectable") auto-repeat
        // behavior can deliver a KeyRelease immediately followed by a
        // KeyPress for the *same* physical keystroke while a key is held -
        // indistinguishable, from this client's point of view, from two
        // genuinely separate presses. `convert_key_press_event`/
        // `convert_key_release_event` (see keyboard.rs) have no way to
        // recover that distinction after the fact, so it has to be fixed
        // here, at the X server connection level, before any key events
        // are ever received. SpectralPrism fix - see this vendored copy's
        // README.md.
        unsafe {
            xlib::XkbSetDetectableAutoRepeat(dpy, 1, std::ptr::null_mut());
        }
