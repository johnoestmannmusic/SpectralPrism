...
impl<'a> Window<'a> {
        ...
        B: Send + 'static,
    {
        ...
        // Actively request keyboard focus for the newly mapped window.
        // Without this, nothing in `baseview` ever asks the X server for
        // input focus - the window only ends up focused as an indirect
        // side effect of e.g. some other native dialog closing and the
        // window manager's own default transient-child focus-return
        // behavior kicking in, which meant text fields in this window
        // could be completely untypable until that happened at least once.
        // SpectralPrism fix - see this vendored copy's README.md.
        unsafe {
            x11::xlib::XSetInputFocus(
                xcb_connection.dpy,
                window_id as std::ffi::c_ulong,
    ...
    // Both were previously `unimplemented!()` (would panic if ever called)
    // - implemented for real as part of the same focus fix as the
    // `XSetInputFocus` call in `open_parented`/`open_blocking` above.
    // SpectralPrism fix - see this vendored copy's README.md.
    pub fn has_focus(&mut self) -> bool {
        unsafe {
            let mut focused: x11::xlib::Window = 0;
            let mut revert_to: i32 = 0;
            x11::xlib::XGetInputFocus(self.inner.xcb_connection.dpy, &mut focused, &mut revert_to);
            focused as u32 == self.inner.window_id
        }
    }
    ...
    pub fn focus(&mut self) {
        unsafe {
            x11::xlib::XSetInputFocus(
                ...
                self.inner.window_id as std::ffi::c_ulong,
                x11::xlib::RevertToParent,
                x11::xlib::CurrentTime,
            );
        }
